...
def run_queries_and_export(connection: oracledb.Connection, query_name_to_use: str, output_filename: str):
  ...
  all_dataframes: List[pd.DataFrame] = [] # New list to collect DFs for 'ALL' mode
    ...
    for config in queries_to_run:
      ...
      try:
        ...
        if rows:
          ...
          # Logic Change: Store based on mode
          if is_all_mode:
            all_dataframes.append(df) # Append to list for concatenation
          else:
            individual_results[name] = df # Store individually for separate sheets
  ...
  if is_all_mode:
    if all_dataframes:
      ...
      # Use a single dictionary entry for a single sheet
      results_for_export = {"ALL_QUERIES_COMBINED": combined_df}
    else:
      results_for_export = {}
  else:
    # Use the individual results dictionary for separate sheets
...
if __name__ == "__main__":
  ...
  # Generate dynamic filename based on user request (report-name-timestamp.xlsx)
  timestamp_str = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
  
  # Use 'ALL_QUERIES_COMBINED' or the specific query name in the filename
  if queryNameToUse.upper() == 'ALL':
    name_for_file = 'ALL_QUERIES_COMBINED'
  else:
    # Sanitize name in case it contains characters invalid for filesystems
  ...
  try:
    ...
    # 2. Run Queries and Export if Connection is successful
    if db_connection:
      # Pass the dynamically generated filename
